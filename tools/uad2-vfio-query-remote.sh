...
case "$RUN_MODE" in
    ""|experiment-010)
        ;;
    *)
        echo "refusing: unsupported query experiment option" >&2
        exit 1
        ;;
esac

if [ "$#" -gt 1 ]; then
    echo "refusing: too many query experiment options" >&2
    exit 1
fi
...
if [ ! -x "$PROBE" ]; then
    echo "refusing: query experiment binary is missing" >&2
    exit 1
fi
...
if [ "$RUN_MODE" = "experiment-010" ]; then
    "$PROBE" --with-interrupt-gates
else
    "$PROBE"
fi
