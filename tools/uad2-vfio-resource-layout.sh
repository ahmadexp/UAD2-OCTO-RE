...
PROBE="/tmp/uad2-vfio-resource-layout"

if [ "$#" -ne 0 ] || [ "$(id -u)" -ne 0 ]; then
    echo "refusing: no arguments are accepted and root is required" >&2
    exit 1
fi
...
if [ ! -x "$PROBE" ]; then
    echo "refusing: resource-layout snapshot binary is missing" >&2
    exit 1
fi
