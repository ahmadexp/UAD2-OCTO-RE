...
The current repository has no completed general-purpose UAD program and no GPU
benchmark. The manuscript defines the workloads, baselines, metrics, sampling
plan, and decision rule needed to make a defensible comparison after execution
is available.
...
1. Receive a valid runtime response from the card.
