...
\begin{@twocolumnfalse}
  ...
  initializes all command and response rings, receives a valid Linux-controlled
  framework response, and submits one integrity-protected resource independently
  to all eight DSPs. It also completes the captured 13-resource RealVerb pass
  on DSP0. Static analysis recovers the host runtime address-patch
  and resource-readback builders, although a bounded standalone readback
  produced no response. The inner executable, DSP-side relocation, entry-point,
  and buffer rules remain unresolved, so it has not completed a general-purpose
  program.
  ...
  \statusbox{The repository contains no completed UAD program and no completed
  UAD-versus-GPU comparison. All comparative outcomes in this manuscript are
  hypotheses. A results paper must replace this status with
...
At present, the first contribution's transport, valid-response, authenticated
resource-loader targeting, and empty-engine recovery components are established.
Entry-point execution and bounded program output are not \cite{uad-project}.
...
Host runtime address patch and readback ABI & Confirmed & The official host resolves selected-resource offsets through \texttt{0x00150000} and constructs \texttt{0x000c0004} readbacks; a standalone bounded request produced no response. \\
...
Static analysis of the plug-in runtime further identifies a fixed
\texttt{0x0af0}-byte allocation record, 16-byte memory-spec entries, 8-byte
readback specs, command class \texttt{0x00150000} for mapped-address patching,
and command \texttt{0x000c0004} for resource readback. A Linux trial issued the
fixed four-dword readback at the accepted \texttt{0x12b} mapping. The command
was consumed, but its six-dword response descriptor remained untouched through
the deadline. Thus the host ABI is known, while a complete \texttt{Process}
transaction or activation state remains a prerequisite candidate
\cite{uad-project}.

A subsequent bounded replay submitted the complete first RealVerb resource
pass. All 13 objects returned exact resource- and command-correlated
\texttt{0x80070004} completions in 1 to 5 ms, including three objects carried
by two page-bounded DMA descriptors. A fixed readback after the thirteenth
success was again consumed without a response. Incomplete resource loading is
therefore ruled out, but no process entry or output is established
\cite{uad-project}.

The complete-pass state was not reusable. A later resource reload and query
\texttt{0x00260001} were consumed without responses. All 13 exact pool-zero
unload commands were then consumed, and the isolated per-DSP reset sequence
passed, but the runtime service remained unanswered. No private-resource
zeroing or memory-spec update was submitted. Fresh official plug-in activation
is therefore a prerequisite for the next allocation trial
\cite{uad-project}.
...
waiting. A valid response and authenticated resource completion are no longer
the blockers, and the complete captured resource pass now succeeds. The
remaining gate is a decoded OCTO executable with proven
segments, DSP-side relocations, entry point, reserved ranges, and
program-visible buffer semantics. The recovered host address-patch layer does
not satisfy this gate. It must first run as a bounded DSP0 heartbeat, followed by the affine
transform. The results section must remain empty of performance claims until
those gates pass.

The current live card also requires official runtime reactivation before the
allocation experiment can resume. Transport readiness and reset success are
not treated as application-service readiness.
...
reset paths. Authenticated loader targeting is now proven on all eight engines:
the selected engine consumes the exact object while all seven non-target ring
index sets remain unchanged. Program-level execution isolation is not yet
proven. After correctness and single-DSP performance are established, each DSP
is tested under normal
...
The paper is ready to accumulate comparative measurements only after a harmless
program produces bounded output and the result survives reset and IOMMU
cleanup. Valid Linux runtime and resource-success responses have now been
observed, body-wide integrity enforcement is demonstrated, and authenticated
loader targeting passes on all eight DSPs. All 13 resources in the captured
RealVerb pass also succeed on DSP0. The host runtime address-patch and
readback builders are also recovered, but bounded standalone readback did not
produce a response, and the current runtime requires fresh official activation.
Until the inner executable, DSP-side relocation,
entry-point, and buffer rules are recovered, the defensible finding
is the transport, resource-loader, and isolation foundation, not performance
superiority.
