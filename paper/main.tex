...
\begin{@twocolumnfalse}
  ...
  to all eight DSPs. The inner executable, relocation, entry-point, and buffer
  rules remain unresolved, so it has not completed a general-purpose program.
...
Authenticated resource transport & Confirmed & Exact resource \texttt{0x12b} succeeds on DSP0 through DSP7; sampled one-bit changes across its opaque body are rejected. \\
...
the blockers. The remaining gate is a decoded OCTO executable with proven
segments, relocations, entry point, reserved ranges, and program-visible buffer
semantics. It must first run as a bounded DSP0 heartbeat, followed by the affine
...
loader targeting passes on all eight DSPs. Until the inner executable,
relocation, entry-point, and buffer rules are recovered, the defensible finding
