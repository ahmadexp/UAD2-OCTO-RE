...
Valid runtime response & Confirmed, partial & The official plug-in path returns a stable 768-entry table and exact intermediate successes for two form-zero resources. \\
...
and shared one 768-dword body containing four state values. The same run
returned the statically predicted \texttt{0x80070004} intermediate-success form
for form-zero resources \texttt{0x120} and \texttt{0xd0}, including exact
resource IDs and envelope command words. RealVerb-Pro instantiated but the
official host disabled it with error \texttt{-38}, the recovered all-zero
resource-response result. These observations validate partial DSP-side resource
acceptance, not a complete or correct program \cite{uad-project}.
...
cleanup. Valid runtime and resource-success responses have now been observed,
but the complete official plug-in load still terminates in the all-zero
response path. Until that later boundary is resolved, the defensible finding is
the recovered transport, partial loader validation, and isolation foundation,
not performance superiority.
