...
\begin{@twocolumnfalse}
  ...
  initializes all command and response rings, loads the exact authenticated
  13-resource RealVerb allocation, and submits a bounded 64-sample stereo
  buffer job independently to all eight DSPs. Input samples return bit-for-bit
  with counter- and channel-correlated completion records, while non-target
  ring indices remain unchanged. The Linux ABI now implements owned buffers,
  authorized program loading, synchronous submission, and completion retrieval.
  A Process-coupled readback returns a valid response, and a complete
  430-dword private-object snapshot verifies every host relocation target.
  The inner executable, authentication or encryption algorithm, DSP-side
  relocation, entry point, and runtime reservations remain unresolved, so the
  stack cannot load user-authored general-purpose code.
  Therefore, this version is a study design and does not claim a measured UAD
  performance advantage. Its central contribution is a falsifiable protocol
  that can support such a claim if, and only if, the resulting data satisfy the
  preregistered criteria.
  \end{abstract}

  \statusbox{The repository contains a completed authorized UAD buffer job but
  no user-authored general-purpose program and no completed UAD-versus-GPU
  comparison. All comparative outcomes in this manuscript are hypotheses. A
...
\paragraph{Contributions required for the comparison.}
...
At present, the first contribution is established for an authorized official
allocation: the stack performs authenticated loading, bounded input and output,
completion correlation, per-engine targeting, and recovery. User-authored
entry-point execution remains unavailable because the inner program format and
authentication rules are unresolved \cite{uad-project}.
...
Valid runtime response & Confirmed & Linux query 026 returns the exact five-dword response form in 1 ms; the official path also returns a decoded 768-entry authorization table. \\
Authenticated resource transport & Confirmed & Exact resource \texttt{0x12b} succeeds on DSP0 through DSP7; all 13 resources in the captured RealVerb pass succeed on DSP0; sampled one-bit changes are rejected. \\
Host allocation and Process ABI & Confirmed & The official host stores the first private mapped resource at instance offset \texttt{0xbd8} and places that address in word 3 of command \texttt{0x000b0004}. \\
Private Process readback & Confirmed & The native \texttt{0x0af8}-byte allocation record declares one readback; Linux receives \texttt{0x80010006} and snapshots all 430 known private-object dwords. \\
Authorized bounded buffer execution & Confirmed & A 64-sample stereo job completes with exact zero and opposed half-scale impulse round trips independently on DSP0 through DSP7. \\
User-authored general-purpose program & Not achieved & The captured official program is not a controlled implementation of a benchmark kernel, so no matched UAD workload can yet be compared for speed. \\
GPU comparison dataset & Not achieved & A preliminary affine baseline does not provide a matched UAD result, power trace, or crossover dataset. \\
\bottomrule
\end{tabularx}
\end{table*}

An isolated Windows reference run used the exact UAD 11.0.1 media, Secure
Boot, TPM, and the signed OCTO driver. The official updater's extended header,
624 full-page payload descriptors, 2,192-byte tail descriptor, and response
descriptor were all consumed. After RTC-backed cold recovery, the driver again
published all 16 rings and enabled all eight DSP engines. Pausing at the exact
response-read-index boundaries showed zero-filled target pages, so descriptor
consumption is not counted as a valid runtime response or program result
\cite{uad-project}.

A subsequent official plug-in-host experiment crossed that boundary. Four
responses used header \texttt{0x80030302}, carried request-correlated tokens,
and shared one 768-dword body. Static analysis maps its wire values to Demo not
started, Authorized, Authorized, and Auth update required. The two authorized
encodings collapse to the same host-visible state. RealVerb-Pro then reached an
all-zero response for its first captured object, resource \texttt{0x12b}.

Linux later adopted the resident framework with fresh IOMMU mappings. Query
\texttt{0x00260001} returned header \texttt{0x800c0005} in 1 ms. The exact
460-byte \texttt{0x12b} object returned
\texttt{80070004 00000000 0000012b 00010073}, also in 1 ms. Single-bit changes
at both ends of its 48-byte preserved prefix and at four locations in its
384-byte trailing core all produced structured rejection statuses. Finally,
the unchanged object succeeded independently on DSP0 through DSP7 with
target-specific interrupt masks and no non-target ring-index movement. These
results prove response transport, whole-body integrity enforcement, and loader
targeting. They do not reveal the clear executable or activate an entry point
\cite{uad-project}.

Static analysis and two repeatable live captures identify a fixed
\texttt{0x0af8}-byte allocation record, 16-byte memory-spec entries, 8-byte
readback specs, and mapped-address patching through command class
\texttt{0x00150000}. The complete first RealVerb allocation contains 13
authenticated objects. Every object returns a resource- and command-correlated
\texttt{0x80070004} completion in 1 to 5 ms, including three objects carried by
two page-bounded DMA descriptors. There is consequently no failing object in
the captured RealVerb resource sequence \cite{uad-project}.

Hash-locked static analysis also resolves the missing Process pointer. The
official runtime stores the mapped address of the first private allocation at
plug-in-instance offset \texttt{0xbd8}; for the captured allocation that address
is \texttt{0x0009d00a}. It places the address in word 3 of
\texttt{0x000b0004 0x00400000 <request-id> <private-address>}. Earlier trials
used the first public allocation and are retained only as negative controls:
their zero and impulse outputs were identical and therefore were not evidence
of execution \cite{uad-project}.

With the corrected pointer, a zero input produces a zero output and an opposed
half-scale stereo impulse returns bit-for-bit through the 64-sample buffers. The
completion begins \texttt{0x80020044} and correlates the request counter and
channel; seven following zero ticks also return zero. Repeating the complete
allocation and job independently on DSP0 through DSP7 leaves all non-target
ring indices unchanged. A one-byte authenticated-resource mutation is rejected,
after which the exact bundle succeeds without a cold reboot. These results close
the official program-buffer path, but they neither expose the encrypted inner
executable nor establish a user-controlled DSP entry point \cite{uad-project}.

The native record declares one four-dword readback from private resource zero
at offset 419. Reproducing it inside Process changes the main flags from
\texttt{0x00400000} to \texttt{0x00400002} and returns
\texttt{80010006 000001a3 00000000 00000000 00000000 00000000}. A second
bounded request recovers all 430 known dwords of the first private resource.
Exactly 32 are nonzero, their offsets equal all 32 later memory-spec patch
destinations, and their values equal the corresponding allocation addresses.
The other 398 dwords are zero. This proves the Process object is a host-patched
runtime control block rather than a clear executable. A 64-dword negative
control at the first public Bill allocation consumes its command but leaves the
response canary unchanged \cite{uad-project}.

Analog Devices' ordinary SHARC loader format uses three-word block headers with
documented final, zero-fill, and initialized-data tags
\cite{adi-cces-loader}. A metadata-only scan tested every byte offset of all 26
post-transform RealVerb Bill bodies under both 32-bit word endiannesses. None
of 45,398 offsets forms a plausible standard loader header. The internal RAM
regions documented for ADSP-21469 include the address ranges used by the live
private and public allocations \cite{adi-adsp21469-ezboard}. These results rule
out clear ordinary LDR framing in this corpus, but do not identify one specific
...
No comparative result exists yet. ABI version 2 now implements coherent input
and output buffers, exact authorized RealVerb loading, synchronous submission,
and completion retrieval. The captured allocation completes and one opposed
half-scale stereo impulse returns exactly through 64-sample buffers on every
DSP. This closes the host buffer semantics needed for the official workload.
The exact private readback specification, full runtime control object, and all
32 outer relocation destinations are also verified dynamically.
The remaining gate is a user-authored OCTO executable with proven segments,
DSP-side relocations, entry point, reserved ranges, and lawful authentication.
The captured official job cannot substitute for the affine benchmark because
its instruction stream is not controlled by the experiment. The results
section must remain empty of performance claims until a matched custom
transform and the baselines pass correctness checks.
...
The OCTO exposes eight engines with independently exercised reset paths. Normal
authorized program-buffer execution is now proven on all eight engines: the
selected engine completes an exact input-dependent output while all seven
non-target ring-index sets remain unchanged. A mutated authenticated object is
rejected, the card recovers, and an unchanged retry succeeds. Hostile custom
program isolation is not yet testable. After a user-authored transform is
available, each DSP is tested under program timeout, response timeout, process
exit, and an IOMMU-contained fault. Status, ring indices, and output buffers for
all other DSPs are recorded before and after each case.
...
The authorized RealVerb allocation now produces bounded, input-dependent output
and survives reset and IOMMU cleanup on all eight targets. The host runtime
address-patch, first-private-resource Process pointer, buffer objects, request
correlation, completion record, private readback, complete 430-dword control
object, and normal target isolation are recovered. A
one-byte resource mutation is rejected and recovery permits an immediate exact
retry. The private object contains allocation pointers rather than code, the
public Bill pool is not readable through this operation, and the transmitted
Bill bodies do not expose standard SHARC loader headers. Until the inner
executable, authentication or encryption algorithm,
DSP-side relocation, entry point, and runtime reservations are recovered, the
defensible finding is an authorized-workload execution and isolation
foundation, not general-purpose performance superiority.
