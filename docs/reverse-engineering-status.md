...
| Understand firmware and plug-in container formats | Fixed FBUT/GBUT/HBUT wrapper, all 47 artifacts, full official HBUT transport, `Bill` outer header, conditional transform, envelope, allocator, copy limit, and completion parser are recovered. The exact first RealVerb object is accepted from Linux. | Decode the opaque HBUT payload and the authenticated `Bill` cleartext. |
| Determine signing, authentication, relocation, and loading rules | Device-side integrity or authentication over both the 48-byte prefix and 384-byte core is dynamically proven. Exact host mappings for pre-completion errors `0x0005` and `0x000d` are recovered. | Identify the algorithm and key source, then recover clear segments, relocation rules, entry point, and reservations. The current evidence proves enforcement, not its construction. |
| Load a harmless DSP0 program | The exact authenticated `0x12b` program resource is accepted from Linux, including its success response, but no module entry point or bounded output was activated. | Obtain a lawful clear executable and OCTO entry ABI, then run a bounded heartbeat with a recovery oracle. |
| Implement host-to-DSP buffers and completion handling | Ring DMA, valid query response, resource completion, and all-eight loader targeting are reconstructed and observed. Public compute calls remain fail-closed. | A program-visible IOVA contract, one bounded buffer exchange, and job completion before enabling UAPI job calls. |
...
container. The official HBUT experiment resolved the host transport but not
the payload or persistence mechanism.

This is an evidence gate, not an assertion that the remaining work is
impossible. Operation `0x6f`, another firmware update, and the former first
all-zero target are no longer on the critical path. The remaining critical
artifact is a lawful cleartext representation of one authenticated core, or a
DSP-side loader trace that reveals its decoded segments and entry ABI.
